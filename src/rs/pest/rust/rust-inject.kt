...
class InlineGrammarInjector : LanguageInjector {
	override fun getLanguagesToInject(host: PsiLanguageInjectionHost, places: InjectedLanguagePlaces) {
		if (host !is RsLitExprMixin) return
		if (isInlineGrammar(host)) places.addPlace(PestLanguage.INSTANCE, TextRange(1, host.textLength - 1), null, null)
	}

	/// Do we need to check the `#[derive(Parser)]` as well?
	private fun isInlineGrammar(host: RsLitExprMixin): Boolean {
		val parent = host.parent as? RsMetaItemImplMixin ?: return false
