...
fun livePreview(file: PestFile) {
	...
	val psiFile = PsiManagerEx.getInstanceEx(project).findFile(virtualFile) ?: return
...
class LivePreviewElement(node: ASTNode) : ASTWrapperPsiElement(node) {
	companion object Tokens {
		@JvmField val TYPE = IElementType("LivePreview Content", LivePreviewLanguage.INSTANCE)
	}
}
